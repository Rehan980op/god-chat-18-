// server.js - Final Complete Version with All Rules
...
app.post("/api/chat", upload.single("file"), async (req, res) => {
  try {
    ...
    // ✅ Agar bilkul empty hai aur file bhi nahi hai → AI ko mat bhejo
    if (message.trim() === "" && !file) {
      return res.json({ reply: "⚠️ Please type a message before sending." });
    }

    // Agar file bheji gayi hai to uska content extract karo
    ...
    if (file) {
      ...
      fs.unlinkSync(file.path); // cleanup after reading
    ...
    if (containsGaali(message)) {
      if (containsRehan(message)) {
        // Agar message me Rehan bhi hai aur gaali bhi hai
        return res.json({
    ...
    // === Agar upar koi custom rule match nahi hua, tabhi AI call karo ===
    const systemPrompt = [{
      role: "system",
